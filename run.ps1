...
function Assert-Deposit  { if (-not $env:DEPOSIT_WEI) { Fail "DEPOSIT_WEI is required" } }
...
function Get-NextSalt([string]$s) {
...
function Invoke-Status([string]$arg) {
    ...
    while ($miss -lt 2 -and $found -lt 100) {
        ...
        if (-not $code -or $code.Trim().Length -le 2) { $miss++; $salt = Get-NextSalt $salt; continue }
        $miss = 0; $found++
        Show-Agent $salt $h
